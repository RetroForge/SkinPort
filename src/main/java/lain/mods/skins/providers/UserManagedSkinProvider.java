...
public class UserManagedSkinProvider implements ISkinProvider {
    ...
    public ISkin getSkin(IPlayerProfile profile) {
        ...
        SharedPool.execute(() -> {
            byte[] data = null;
            if (!Shared.isOfflinePlayer(profile.getPlayerID(), profile.getPlayerName())) data = readFile(
                _dirU,
                "%s.png",
                profile.getPlayerID()
                    .toString()
            ...
            if (data != null) skin.put(data, SkinData.judgeSkinType(data));
        });
        return skin;
    }
    ...
    private byte[] readFile(File dir, String filename) {
        byte[] contents;
        if ((contents = Shared.blockyReadFile(new File(dir, filename), null, null)) != null
            && SkinData.validateData(contents)) return contents;
        return null;
    }
