...
public class SkinBundle implements ISkin {
    ...
    protected Optional<ISkin> find() {
        ...
        if ((skins = ref.get()).isEmpty()) return Optional.empty();
        return skins.stream()
    ...
    public ByteBuffer getData() {
        return find().orElse(SkinProviderAPI.DUMMY)
            .getData();
    }
    ...
    public SkinBundle set(Collection<ISkin> c) {
        ...
        Collection<ISkin> skins;
        if (!(skins = ref.getAndSet(c)).isEmpty()) skins.forEach(ISkin::onRemoval);
        return this;
    }
    ...
    public boolean setRemovalListener(Consumer<ISkin> listener) {
        ...
        if (listeners.add(listener)) {
            Collection<ISkin> skins;
            if (!(skins = ref.get()).isEmpty()) skins.forEach(e -> e.setRemovalListener(listener));
            return true;
        }
        return false;
    }
    ...
    public boolean setSkinFilter(Function<ByteBuffer, ByteBuffer> filter) {
        ...
        if (filters.add(filter)) {
            Collection<ISkin> skins;
            if (!(skins = ref.get()).isEmpty()) skins.forEach(e -> e.setSkinFilter(filter));
            return true;
        }
        return false;
    }
