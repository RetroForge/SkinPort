...
public class SkinProviderAPI {
     ...
     *         if the profile got updated during the lifetime of a SkinBundle, new ISkin objects will be gathered and a
     *         thread will be used to monitor those objects to wait their isDataReady for up to 10 seconds before
     *         updating the SkinBundle.
     */
    public static ISkinProviderService create() {
        return new ISkinProviderService() {

            private final LoadingCache<SkinBundle, AtomicReference<Object>> reloading = CacheBuilder.newBuilder()
                .weakKeys()
                .build(new CacheLoader<SkinBundle, AtomicReference<Object>>() {
            ...
            private final LoadingCache<IPlayerProfile, SkinBundle> cache = CacheBuilder.newBuilder()
                .expireAfterAccess(15, TimeUnit.SECONDS)
                .removalListener(new RemovalListener<IPlayerProfile, SkinBundle>() {

                    @Override
                    public void onRemoval(RemovalNotification<IPlayerProfile, SkinBundle> notification) {
                        SkinBundle skin = notification.getValue();
                        if (skin != null) skin.onRemoval();
                    }
                ...
                .build(new CacheLoader<IPlayerProfile, SkinBundle>() {
                    ...
                    public SkinBundle load(IPlayerProfile key) throws Exception {
                        ...
                        return new SkinBundle().set(
                            providers.stream()
                                .map(provider -> { return provider.getSkin(key); })
                                .filter(skin -> { return skin != null; })
                    ...
                    public ListenableFuture<SkinBundle> reload(IPlayerProfile key, SkinBundle oldValue)
                        ...
                        Object token;
                        reloading.getUnchecked(oldValue)
                            .set(token = new Object());
                        long deadline = System.currentTimeMillis() + 10000; // 10 seconds
                        Supplier<Boolean> ready = () -> {
                            return System.currentTimeMillis() - deadline > 0L || reloading.getUnchecked(oldValue)
                                .get() != token
                                || skins.stream()
                                    .filter(ISkin::isDataReady)
                                    .findAny()
                                    .isPresent();
                        };
                        Runnable update = () -> {
                            if (reloading.getUnchecked(oldValue)
                                .compareAndSet(token, null)) oldValue.set(skins);
                        };
            ...
            private final Consumer<IPlayerProfile> profileChangeListener = profile -> {
                if (cache.getIfPresent(profile) != null) cache.refresh(profile);
            };
