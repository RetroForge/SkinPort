...
public class Shared {
    ...
    public static final GameProfile DUMMY = new GameProfile(
        UUID.fromString("ae9460f5-bf72-468e-89b6-4eead59001ad"),
        "");

    private static final Cache<UUID, Boolean> offlines = CacheBuilder.newBuilder()
        .weakKeys()
        .build();
     ...
     * @param consumer     a consumer that will receive a Throwable if the task failed during the call, null is
     *                     acceptable.
    ...
    public static <T> T blockyCall(Callable<T> callable, T defaultValue, Consumer<Throwable> consumer) {
        if (callable == null) return defaultValue;
     ...
     * @param consumer        a consumer that will receive a Throwable if failed during reading the file, null is
     *                        acceptable.
    ...
    public static byte[] blockyReadFile(File file, byte[] defaultContents, Consumer<Throwable> consumer) {
        if (file == null) return defaultContents;
        return blockyCall(() -> {
    ...
    private static Optional<byte[]> readAndDelete(Optional<Path> path) {
        try (FileChannel channel = FileChannel.open(
            path.orElseThrow(FileNotFoundException::new),
            StandardOpenOption.READ,
            StandardOpenOption.DELETE_ON_CLOSE); ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            channel.transferTo(0L, Long.MAX_VALUE, Channels.newChannel(baos));
