...
public class PlayerProfile implements IPlayerProfile {
    ...
    private static final LoadingCache<GameProfile, PlayerProfile> profiles = CacheBuilder.newBuilder()
        ...
        .build(new CacheLoader<GameProfile, PlayerProfile>() {
            ...
            public PlayerProfile load(GameProfile key) throws Exception {
                ...
                } else if (Shared.isOfflinePlayer(key.getId(), key.getName())) // an offline profile that needs
                                                                               // resolving
                {
                    Futures.addCallback(MojangService.getProfile(key.getName()), new FutureCallback<GameProfile>() // resolve
                                                                                                                   // it
                    {
                        ...
                        public void onSuccess(GameProfile resolved) {
                            ...
                            Futures.addCallback(MojangService.fillProfile(resolved), new FutureCallback<GameProfile>() // fill
                                                                                                                       // it
                            {
                ...
                } else if (key.getProperties()
                    .isEmpty()) // an assumed online profile that needs filling
                {
                    Futures.addCallback(MojangService.fillProfile(key), new FutureCallback<GameProfile>() // fill it
                    {
                        ...
                        public void onSuccess(GameProfile filled) {
                            ...
                            Futures
                                .addCallback(MojangService.getProfile(key.getName()), new FutureCallback<GameProfile>() // resolve
                                                                                                                        // it
                            {
                                    ...
                                    public void onSuccess(GameProfile resolved) {
                                        ...
                                        Futures.addCallback(
                                            MojangService.fillProfile(resolved),
                                            new FutureCallback<GameProfile>() // fill it
                                        {
    ...
    public boolean equals(Object o) {
        ...
        if ((p = _profile.get()) == null) // gc
            return false;
        if (o instanceof PlayerProfile) return p.equals(((PlayerProfile) o)._profile.get());
        return false;
    }
    ...
    private synchronized void set(GameProfile profile) {
        ...
        for (Consumer<IPlayerProfile> l : _listeners) l.accept(this);
    }

    @Override
    public boolean setUpdateListener(Consumer<IPlayerProfile> listener) {
        if (this == DUMMY) return false;
        if (listener == null || _listeners.contains(listener)) return false;
