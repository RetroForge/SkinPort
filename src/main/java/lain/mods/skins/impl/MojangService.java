...
public class MojangService {
        ...
        .build(new CacheLoader<GameProfile, Optional<GameProfile>>() {
            ...
            public Optional<GameProfile> load(GameProfile key) throws Exception {
                ...
                if (key.isComplete() && !key.getProperties()
                    .isEmpty()) // already filled
                    return Optional.of(key);
                GameProfile filled = Shared.blockyCall(() -> {
                    return MinecraftUtils.getSessionService()
                        .fillProfileProperties(key, false); // fill it
                }, key, null);
                if (filled == key) // failed
                    return Optional.empty();
                if (!filled.isComplete() || filled.getProperties()
                    .isEmpty()) // partially filled, this won't happen in current implementation, it's here just in
                                // case.
                    return Optional.empty();
            ...
            public ListenableFuture<Optional<GameProfile>> reload(GameProfile key, Optional<GameProfile> oldValue)
                ...
                return Shared.submitTask(() -> { return load(key); });
            }
        ...
        .build(new CacheLoader<String, Optional<GameProfile>>() {
            ...
            public Optional<GameProfile> load(String key) throws Exception {
                ...
                return Optional.ofNullable(Shared.blockyCall(() -> {
                    return makeRequest(String.format("https://api.mojang.com/users/profiles/minecraft/%s", key)); // request
                                                                                                                  // it
                }, null, null));
            }
            ...
            private GameProfile makeRequest(String request) throws IOException {
                ...
                if (code == 204 || code == 404) // not found
                    ...
                    try (InputStream in = conn.getInputStream()) {
                        ...
                        if (!constructed.isComplete()) // why does the server return an incomplete profile? treat it as
                                                       // not found.
                            return Shared.DUMMY;
                        if (Shared.isOfflinePlayer(constructed.getId(), constructed.getName())) // why does the server
                                                                                                // return an offline
                                                                                                // profile? treat it as
                                                                                                // not found.
                            return Shared.DUMMY;
                        return new GameProfile(constructed.getId(), constructed.getName()); // reconstruct it because
                                                                                            // default JsonDeserializer
                                                                                            // doesn't construct a
                                                                                            // GameProfile properly,
                                                                                            // can't use
            ...
            public ListenableFuture<Optional<GameProfile>> reload(String key, Optional<GameProfile> oldValue)
                ...
                if (oldValue.isPresent()) {
                    if (oldValue.get() == Shared.DUMMY) return Futures.immediateFuture(Optional.empty()); // effectively
                                                                                                          // schedule a
                                                                                                          // refresh in
                                                                                                          // next
                                                                                                          // reload.
    ...
    public static ListenableFuture<GameProfile> fillProfile(GameProfile profile) {
        if (profile == null) return Futures.immediateFailedFuture(new NullPointerException("profile must not be null"));
        ...
        return Shared.submitTask(
            () -> {
                return filledProfiles.getUnchecked(profile)
                    .orElse(profile);
            });
    }
    ...
    public static ListenableFuture<GameProfile> getProfile(String username) {
        ...
        return Shared.submitTask(
            () -> {
                return resolvedProfiles.getUnchecked(username)
                    .orElse(Shared.DUMMY);
            });
    }
