...
public class SkinPortGuiCustomizeSkin extends GuiScreen {
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        if (button.id == 200) {
            ...
            else SkinCustomization.ClientFlags |= a.part.getFlag();
    ...
    public void initGui() {
        ...
        if (i % 2 == 1) i++;
        buttonList.add(
            new GuiButton(200, width / 2 - 100, height / 6 + 24 * (i >> 1), I18n.format("gui.done", new Object[0])));
    }

    private String printButtonText(SkinCustomization part) {
        return part.getDisplayName()
            .getFormattedText() + ": "
            + (SkinCustomization.contains(SkinCustomization.ClientFlags, part)
