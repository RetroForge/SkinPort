                ...
                .readLines(new LineProcessor<Void>() {
                    ...
                    public boolean processLine(String line) throws IOException {
                        if (line.startsWith("PK: ")) {
                            ...
                            if (data.length == 2) PackageMap.put(data[0], data[1]);
                        } else if (line.startsWith("CL: ")) {
                            line = line.substring(4);
                            String[] data = line.split(" ");
                            if (data.length == 2) ClassMap.put(data[0], data[1]);
                            ...
                            if (data.length == 2) {
                                ...
                                if (i0 != -1 && i1 != -1) FieldMap.put(
                                    ImmutableList
                                        .of(data[0].substring(0, i0), data[0].substring(i0 + 1, data[0].length())),
                                    ImmutableList
                                        .of(data[1].substring(0, i1), data[1].substring(i1 + 1, data[1].length())));
                            }
                            ...
                            if (data.length == 4) {
                                ...
                                if (i0 != -1 && i2 != -1) MethodMap.put(
                                    ImmutableList.of(
                                        data[0].substring(0, i0),
                                        data[0].substring(i0 + 1, data[0].length()),
                                        data[1]),
                                    ImmutableList.of(
                                        data[2].substring(0, i2),
                                        data[2].substring(i2 + 1, data[2].length()),
                                        data[3]));
                            }
                        }
                        return true;
