...
public class ObfHelper {
    ...
    public String getData(int index) {
        if (index >= 0 && index <= 2) return data[index];
        return null;
    }
    ...
    public boolean match(Object... obj) {
        switch (type) {
            case 1:
                if (obj.length == 1) return data[0].equals(obj[0]);
                break;
            case 2:
                if (obj.length == 1) return data[0].equals(obj[0]);
                break;
            case 3:
                if (obj.length == 1) return data[1].equals(obj[0]);
                else if (obj.length == 2) return data[0].equals(obj[0]) && data[1].equals(obj[1]);
                break;
            case 4:
                if (obj.length == 2) return data[1].equals(obj[0]) && data[2].equals(obj[1]);
    ...
    private void transform(boolean deobfuscated) {
        switch (type) {
            case 1:
                ...
                if (map1.containsKey(data[0])) data[0] = map1.get(data[0]);
                break;
            case 2:
                Map<String, String> map2 = deobfuscated ? Setup.ClassMap : Setup.ClassMap.inverse();
                if (map2.containsKey(data[0])) data[0] = map2.get(data[0]);
                break;
            case 3:
